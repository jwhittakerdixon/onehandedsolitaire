package cards;

public class Deck {

    public Card[] deck;

    public int cardsUsed;

    
    public Deck() {
        this(false);
    }

 
    public Deck(boolean includeJokers) {
        if (includeJokers)
            deck = new Card[54];
        else
            deck = new Card[52];
        int cardCt = 0;
        for ( int suit = 0; suit <= 3; suit++ ) {
            for ( int value = 1; value <= 13; value++ ) {
                deck[cardCt] = new Card(value,suit);
                cardCt++;
            }
        }
        if (includeJokers) {
            deck[52] = new Card(1,Card.JOKER);
            deck[53] = new Card(2,Card.JOKER);
        }
        cardsUsed = 0;
    }

    public void shuffle() {
        for ( int i = deck.length-1; i > 0; i-- ) {
            int rand = (int)(Math.random()*(i+1));
            Card temp = deck[i];
            deck[i] = deck[rand];
            deck[rand] = temp;
        }
        cardsUsed = 0;
    }

    
    public int cardsLeft() {
        return deck.length - cardsUsed;
    }

    public void emptyDeck() {
	cardsUsed = deck.length;
    }

    public void fillDeck() {
	cardsUsed = 0;
    }


    public Card dealCard() {
        if (cardsUsed == deck.length)
            throw new IllegalStateException("No cards are left in the deck.");
        cardsUsed++;
        return deck[cardsUsed - 1];
    }

    public Card dealCard(int i) {
        //if (i > deck.length-cardsUsed)
        //    throw new IllegalStateException("Not enough cards.");
        cardsUsed++;
        return deck[i];
    }

    public void addCard(Card newCard) {
	int suit = newCard.getSuit();
	int value = newCard.getValue();
	int i = findCard(suit,value);
	if (cardsUsed == 0)
        	throw new IllegalStateException("Deck full.");

	Card temp = deck[deck.length-cardsUsed];
	deck[deck.length-cardsUsed]=deck[i];
	deck[i]=temp;

	cardsUsed=cardsUsed-1;
    }

    
    public int findCard(int suit, int value) {
	for (int i = 0; i<deck.length; i++) {
		if (deck[i].getValue()==value) {
			if (deck[i].getSuit()==suit) {
				return i;
			}
		}
	}
	return -1;
    }

    public boolean hasJokers() {
        return (deck.length == 54);
    }

    public void printDeck() {

	for (int i = 0; i<deck.length-cardsUsed; i++) {
		System.out.print(deck[i].toString()+ ", ");
	}
	System.out.println();

   }

} 